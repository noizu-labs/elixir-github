...
defmodule Noizu.Github.Collection.MinimalRepository do
  @moduledoc false
  ...
  def from_json(list, headers) when is_list(list) do
    %__MODULE__{
      items: Enum.map(list, &Noizu.Github.MinimalRepository.from_json(&1)),
      complete: true,
      total: length(list),
  ...
  def from_json(%{items: items} = env, headers) when is_list(items) do
    %__MODULE__{
      items: Enum.map(items, &Noizu.Github.MinimalRepository.from_json(&1)),
